...
class LB130(object):
    ...
    def __init__(self, ip_address):
        ...
        if valid_ip:
            ...
            try:
                ...
                # self.__hue = int(data[col1][col2][col3]["hue"]) This causes an error when the bulb is in off state
                if "dft_on_state" in data[col1][col2][col3].keys():
                    self.__hue = int(data[col1][col2][col3]["dft_on_state"]["hue"])
                    self.__saturation = int(
                        data[col1][col2][col3]["dft_on_state"]["saturation"]
                    )
                    self.__brightness = int(
                        data[col1][col2][col3]["dft_on_state"]["brightness"]
                    )
    ...
    def isOn(self):
        """
        Return true if Light is on else False
        """
        return bool(
            json.loads(self.status())["system"]["get_sysinfo"]["light_state"]["on_off"]
        )
    ...
    def on(self):
        ...
        self.__update(
            ...
            'transition_period":' + str(self.__transition_period) + ',"on_off":1}}}'
        )
    ...
    def reboot(self):
        ...
        """
        self.__update(
            '{"smartlife.iot.common.system":{"reboot":\
                      {"delay":1}}}'
        )
    ...
    def alias(self, name):
        ...
        """
        self.__update(
            '{"smartlife.iot.common.system":{"set_dev_alias"\
                      :{"alias":"'
            + name
            + '"}}}'
        )
    ...
    def time(self, date):
        ...
        if isinstance(date, datetime.datetime):
            self.__update(
                '{"smartlife.iot.common.timesetting":{"set_time"\
                          :{"year":'
                + str(date.year)
                + ',"month":'
                + str(date.month)
